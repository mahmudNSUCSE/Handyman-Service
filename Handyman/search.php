...
<div>
	<div>
		...
		<div class="topnav">
			...
			<a class="active navlink" href="search.php">Search Handyman</a>
			<?php 
			  	...
			  	<form method="post">
					...
					<div class="itemrow">
				  	...
				  	<div class="itemrow">
				  			...
				  			<div style="width: 75%; float: left;" >
				  				<?php echo '<input type="text" style="width: 174px;" name="location" value="'.$f_loca.'" placeholder="e.g: Dhanmondi, Mirpur,..">'; ?>
				  			</div>
				  		</div>
		...
		<div class="nfeedrightsearch">
			<?php
				if (isset($_POST['submit'])){
					$f_job = $_POST['joblistcombo'];
					$f_job = mysqli_real_escape_string($con, $f_job);
					...
					if ($f_job != "None"){
					    $condition = $condition . " AND prefer_job LIKE '%{$f_job}%' ";
					} 
					...
					';
						...
						$query_sub = $con->query("SELECT * FROM `handyman` WHERE ".$condition);
						...
						<div class="p_body">
							<div class="itemrow" style="text-align: center;">
					  				<p><label>'.$query_sub_cnt.' handyman Found </label></p>
					  		</div>
						</div>
					</div>';
					...
					while($row = $query_sub->fetch_assoc()) { 
						...
						$query1 = $con->query("SELECT * FROM user WHERE id='$handyman_id'");
						...
						$cntnm = "";
						if($user != ""){
							$query6 = $con->query("SELECT * FROM applied_post WHERE applied_by='$user' AND applied_to='$handyman_id'");
						...
						echo '
							...
							<div style="float: right;">';
								...
								';
									}elseif($user == $handyman_id){
										
									}else{
										echo '<form action="confirmhandyman.php?confirm='.$handyman_id.'" method="post">
